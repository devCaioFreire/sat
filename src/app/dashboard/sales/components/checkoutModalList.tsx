...
export const CheckoutModalList: React.FC<CheckoutModalProps> = ({ isOpen, onClose, onFormSubmit }) => {
  ...
  const calculateChange = () => {
    const changeValue = calculateTotal() - discountAmount;
    const change = payment - changeValue;
    return selectedPaymentMethod === 'money' ? (change > 0 ? formatValueToTwoDecimals(change) : formatValueToTwoDecimals(0)) : formatValueToTwoDecimals(0);
  };
    ...
    <form
      ...
      <ul className='flex flex-col mt-[calc(2rem+4rem)] h-full justify-between text-lg px-4'>
        ...
        <li
          className='flex justify-between items-center border-b'>
          Desconto %
          <span className='bg-backgroundFields w- px-8 py-2 mb-4 rounded-lg'>
            <input
              type="number"
              id="descount"
        ...
        <li
          className='flex justify-between items-center border-b'>
          Total
          <span className='bg-backgroundFields px-8 py-2 mb-4 rounded-lg'>{formatCurrency(calculateTotal() - discountAmount)}</span>
        </li>
        ...
        {selectedPaymentMethod === 'money' ? (
          <li
            ...
            Troco
            <span
              className='bg-backgroundFields px-8 py-2 mb-4 rounded-lg'>
