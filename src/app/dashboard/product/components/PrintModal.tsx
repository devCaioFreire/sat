...
import ProductsReport, { ProductsReportRef } from "@/hooks/productsReport";
...
export const PrintModal = ({ isOpen, onClose }: CoupomModalProps) => {
  const [productsToPrint, setProductsToPrint] = useState<ProductProps[]>([]);
  const [cupomVisible, setCupomVisible] = useState(true);
  const productsReportRef = React.useRef<ProductsReportRef>(null);
  const { fetchAllProductsForPrint, filterArray } = useProductContext()
    ...
    <div className={`fixed z-50 inset-0 bg-opacity-50 bg-backgroundModal backdrop-blur-md`}>
      <div className="modalContent absolute z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 shadow-2xl flex flex-col w-[32%] h-[83%] gap-4 border border-border rounded-2xl bg-background">
        ...
        <div className="flex w-full h-full mt-2">
          {cupomVisible && <ProductsReport ref={productsReportRef} itens={productsToPrint} />}
        </div>
